...
impl MD5Digest {
    ...
    fn update(&mut self, buffer: &[u8; MD5_BUFFER_BYTE_LENGTH]) {
        ...
        let mut c = self.c;
        let mut d = self.d;
        let w00 = little_endian_word(buffer, 0);
        let w01 = little_endian_word(buffer, 1);
        let w02 = little_endian_word(buffer, 2);
        let w03 = little_endian_word(buffer, 3);
        let w04 = little_endian_word(buffer, 4);
        let w05 = little_endian_word(buffer, 5);
        let w06 = little_endian_word(buffer, 6);
        let w07 = little_endian_word(buffer, 7);
        let w08 = little_endian_word(buffer, 8);
        let w09 = little_endian_word(buffer, 9);
        let w10 = little_endian_word(buffer, 10);
        let w11 = little_endian_word(buffer, 11);
        let w12 = little_endian_word(buffer, 12);
        let w13 = little_endian_word(buffer, 13);
        let w14 = little_endian_word(buffer, 14);
        let w15 = little_endian_word(buffer, 15);
